...
const _supabase = createClient(env.supabaseUrl!, env.supabaseAnonKey!);

// BFN Backend Edge Function initialized
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (path.startsWith('/ipfs')) {
      return await handleIPFS(req, path, corsHeaders);
    }
...
function parseSiweMessage(
  message: string,
): { address: string; nonce: string; domain: string; chainId: number } | null {
  try {
...
async function handleAI(req: Request, path: string, corsHeaders: Record<string, string>) {
  ...
  if (path === '/ai/chat' && req.method === 'POST') {
    ...
    if (ollamaKey) {
      try {
        console.warn('Attempting Ollama Cloud with key:', ollamaKey?.substring(0, 8) + '...');
    ...
    if (!answer) {
      if (!ollamaKey && !deepseekKey && !openaiKey) {
        answer =
          'AI service is not configured. Please set up API keys for OpenAI, DeepSeek, or Ollama.';
      } else {
...
async function handleChainRead(
  req: Request,
  path: string,
...
// IPFS handler using Pinata
async function handleIPFS(req: Request, path: string, corsHeaders: Record<string, string>) {
  const pinataJwt = Deno.env.get('PINATA_JWT');

  if (!pinataJwt) {
    return jsonResponse({ error: 'IPFS service not configured' }, corsHeaders, 503);
  }

  // Upload profile metadata as JSON
  if (path === '/ipfs/profile-metadata' && req.method === 'POST') {
    ...
    try {
      const response = await fetch('https://api.pinata.cloud/pinning/pinJSONToIPFS', {
        ...
        body: JSON.stringify({
          ...
          pinataMetadata: { name: `bfn-profile-${metadata.address || 'unknown'}` },
        }),
      });
      ...
      return jsonResponse(
        { error: 'IPFS upload failed', message: error.message },
        corsHeaders,
        500,
      );
    }
  ...
  if (path === '/ipfs/profile-docs' && req.method === 'POST') {
    try {
      ...
      pinataForm.append('pinataMetadata', JSON.stringify({ name: `bfn-doc-${file.name}` }));
  ...
  if (path.startsWith('/ipfs/') && req.method === 'GET') {
    const hash = path.replace('/ipfs/', '');
    if (!hash || hash === 'profile-metadata' || hash === 'profile-docs') {
      return jsonResponse({ error: 'IPFS hash required' }, corsHeaders, 400);
    }
    ...
    try {
      ...
      return jsonResponse(
        { error: 'Failed to fetch from IPFS', message: error.message },
        corsHeaders,
        500,
      );
    }
