...
export class TransactionsService {
  ...
  async getTransactionHistory(
    ...
    // Ordered by descending block number (Req 12.3)
    ...
    const eventDtos: IndexedEventDto[] = events.map(event => ({
    ...
    // Determine next cursor (block number of the last event, if any)
    const nextCursor = events.length > 0 ? events[events.length - 1]!.blockNumber.toString() : null;
