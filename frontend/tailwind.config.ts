 ...
 *
 * Uses the class-based dark mode strategy so the `ThemeProvider` (next-themes)
 * can toggle themes by setting a class on <html> without a reload (Req 19).
 * The color tokens are wired to CSS variables defined in `globals.css` following
